...
describe('POST /sign-in', () => {
  ...
  test('returns 200 with valid user and password and user is already logged in', async () => {
    const validUser = await validUserFactory();
    const firstLogin = await supertest(app).post('/sign-in').send(validUser);
    const result = await supertest(app).post('/sign-in').send(validUser);
    expect(result.status).toEqual(200);
    expect(result.body).toHaveProperty('token');
    expect(result.body).toHaveProperty('name');
    ...
    expect(result.body).toHaveProperty('address');
    expect(result.body).toHaveProperty('phone');
    expect(firstLogin.status).toEqual(200);
    expect(firstLogin.body).toHaveProperty('token');
    expect(firstLogin.body).toHaveProperty('name');
    expect(firstLogin.body).toHaveProperty('photo');
    expect(firstLogin.body).toHaveProperty('address');
    expect(firstLogin.body).toHaveProperty('phone');
  });
